...
# Build options for Rom tester arduino code
TS_SKETCH := RomTester
TS_BOARD :=  arduino:avr:uno
TS_ASC :=    arduino

# Autogenerated stuff
HPPFILES :=    $(wildcard $(SKETCH)/*.hpp)
CPPFILES :=    $(wildcard $(SKETCH)/*.cpp)
WR_HPPFILES := $(wildcard $(WR_SKETCH)/*.hpp)
WR_CPPFILES := $(wildcard $(WR_SKETCH)/*.cpp)
TS_HPPFILES := $(wildcard $(TS_SKETCH)/*.hpp)
TS_CPPFILES := $(wildcard $(TS_SKETCH)/*.cpp)
...
# ROM tester targets
.PHONY : tester-verify
tester-verify : $(TS_SKETCH)/$(TS_SKETCH).ino $(TS_HPPFILES) $(TS_CPPFILES)
	$(TS_ASC) --verify --board $(TS_BOARD) $<

.PHONY : tester-upload
tester-upload : $(TS_SKETCH)/$(TS_SKETCH).ino $(TS_HPPFILES) $(TS_CPPFILES)
	$(TS_ASC) --upload --board $(TS_BOARD) $<
